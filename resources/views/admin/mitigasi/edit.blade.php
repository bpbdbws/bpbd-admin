...
<div class="row">
    ...
    <div class="col-12">
        <div class="card shadow mb-4">
            ...
            <form class="card-body" method="POST" enctype="multipart/form-data" action="{{route('back-mitigasi.update', $data->id)}}">
                @csrf
                @method('put')
                    ...
                    <div class="col-lg-6">
                         ...
                         <select name="kecamatan" id="kecamatan" class="selectpicker form-control @error('kecamatan') is-invalid @enderror" data-live-search="true" required>
                             ...
                             @foreach ($kecamatan as $item)
                                 <option value="{{ $item->id }}" {{ old('kecamatan',$data->kecamatan_id) == $item->id ? 'selected' : '' }}>{{ $item->kecamatan }}</option>
                             @endforeach
                         </select>
                         @error('kecamatan')
                             <div class="invalid-feedback">
                    ...
                    <div class="col-lg-6">
                     ...
                     <select name="desa" id="desa" class="selectpicker form-control @error('desa') is-invalid @enderror" data-live-search="true" required>
                         ...
                         @foreach ($desa as $item)
                             <option value="{{ $item->id }}" {{ old('desa',$data->desa_id) == $item->id ? 'selected' : '' }}>{{ $item->desa }}</option>
                         @endforeach
                     </select>
                     @error('desa')
                         <div class="invalid-feedback">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="longitude" class="form-control  @error('longitude') is-invalid @enderror" name="longitude" placeholder="Longitude..." value="{{old('longitude', $data->longitude)}}" required readonly>
                    @error('longitude')
                        <div class="invalid-feedback">
                            {{$message}}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="latitude" class="form-control  @error('latitude') is-invalid @enderror" name="latitude" placeholder="Latitude..." value="{{old('latitude', $data->latitude)}}" required readonly>
                    @error('latitude')
                        <div class="invalid-feedback">
                            {{$message}}
                        </div>
...
@section('extraJS')
    ...
    <script>
        ...
        var currentLong = {{$data->longitude}};
